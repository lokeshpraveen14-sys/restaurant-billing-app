...
interface TableNodeProps {
  ...
  onPointerUp: (e: React.PointerEvent, table: Table) => void;
  onClick: (e: React.MouseEvent, table: Table) => void;
}

const TableNode = memo(({
  table, index, isEditMode, draggingTable,
  onPointerDown, onPointerMove, onPointerUp, onClick
}: TableNodeProps) => {
  // Use granular selector for active orders count
  const activeOrdersCount = useOrderStore(s => s.getOrdersByTable(table.id, table.number).length);
...
export default function FloorPlanMap({ tables, isEditMode, onTableClick, onEditTableConfig }: FloorPlanMapProps) {
  const updateTablePosition = useTableStore(s => s.updateTablePosition);
  ...
  }, [isEditMode, draggingTable, dragOffset, updateTablePosition]);

  const handlePointerUp = useCallback((e: React.PointerEvent, table: Table) => {
    if (draggingTable) {
  ...
  const handleClick = useCallback((e: React.MouseEvent, table: Table) => {
    if (isEditMode) {
      if (!hasDragged) {
  ...
  }, [isEditMode, hasDragged, onEditTableConfig, onTableClick]);
